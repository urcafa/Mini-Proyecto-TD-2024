...
#En caso de que no este especificado en el ticket alguna de esta información,
...
arreglar_fruta<- function(fruta){
  ...
  #Separo el vector en dos y creo un dataframe
  ...
  df<- df %>%
    mutate(unidades = as.numeric(substr(frutas, 1,1)),
           ...
           # Borro el número de unidades de la columna
           peso_kg=substr(precios,1,nchar(precios)-20)) %>%
    #Obtengo el peso de la columna precios y creo una columna
    mutate(precio_por_kg=substr(precios,10,14)) %>%
    #Obtengo el pprecio por kgeso de la columna precios y creo una columna
    mutate(precios=substr(precios,nchar(precios)-4,nchar(precios)))
  #Selecciono unicamente el precio de la columna precios
...
detectar_fruta <- function(a){
  ...
  #Para asegurarme de incluir todos los productos selecciono lineas de mas y 
  #borro las sobrantes en este bucle
  ...
  #Detecto los indices de las frutas
  ...
  #Creo un vector con las frutas y sus precios
  ...
  #Quito las frutas del dataframe
...
8) ¿Hay algún patrón de compra según la estacion del año?
