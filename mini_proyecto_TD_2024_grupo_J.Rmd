...
leer_ficheros <- function(df, fichero){
for (i in (1:nrow(df))){
...
lista <- leer_ficheros(df, fichero)
```
# Pregunta 1
...
```{r}
# Definimos las categorías en las que queremos clasificar los productos.
...
add_categoria <- function(lista){
  for (i in (1:length(lista))){
    lista[i] %>% mutate(categoria = categorias)
  }
  return(lista)
}
...
1) ¿Qué se compra más, comida o bebida? 
2) ¿Cuál es el gasto medio de cada compra?
3) ¿En qué momento del día se hacen más compras de media?
