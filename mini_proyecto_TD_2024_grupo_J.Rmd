...
ggsave("graficas/top_5_productos_mas_caros.jpeg", grafica, width = 10, height = 6.5)
```
...
dfIVA <- function(listaArchivos) {
    ...
    for (archivo in listaArchivos) {
        texto <- (readLines(archivo, encoding = "windows-1252"))#Extraigo la informacion de los txt
        ...
        matrizIVA <- matrix(IVA, ncol = 5, byrow = TRUE)#Lo paso a matriz y le doy la forma adecuada para obtener el df que quiero
    ...
    df1 <- df_final %>%#Los datos no estan ordenados por columnas, creo nuevas columnas con la condicion de que cada una sea un tipo de IVA o el total
  rowwise() %>%
  ...
  #Me quedo con las columnas nuevas y selecciono solo los caracteres que me interesan
  df <-df1%>%
  ...
  mutate_all(~ str_replace_all(., ",", "."))%>%#Reenplazo comas por puntos y paso a numerico
...
df_long <- gather(a, key = "Variable", value = "Valor")#Modelo el df en dos columnas para adecuarlo al plot
...
graf<-data.frame(tipo=c("0%","5%","10%","20%"),cantidad=c(sum(a$base0, na.rm = T),sum(a$base5, na.rm = T),sum(a$base10, na.rm = T),sum(a$base21, na.rm = T)))  #Creo un df con los datos que me interesan
graf$tipo <- factor(graf$tipo, levels = c("0%", "5%", "10%", "20%"))#Paso esta columna a factor para que salga ordenada
...
ggsave("graficas/IVA_total.jpeg", IVA1, width = 10, height = 6.5)#Guardo el grafico
```
...
```{r, echo = FALSE}
graf<-data.frame(tipo=c("0%","5%","10%","20%"),cantidad=c(sum(a$imponible0, na.rm = T),sum(a$imponible5, na.rm = T),sum(a$imponible10, na.rm = T),sum(a$imponible21, na.rm = T)))  #Creo un df con los datos que me interesan
graf$tipo <- factor(graf$tipo, levels = c("0%", "5%", "10%", "20%"))#Paso esta columna a factor para que salga ordenada
...
mediasl <- gather(medias, key = "Variable", value = "Valor")#Obtengo las medias y las agrupo en dos columnas
mediasl$Variable <- factor(mediasl$Variable, levels = mediasl$Variable)#Paso  columna a factor para que salga ordenada
mediasl$Categoria <- ifelse(mediasl$Variable %in% c("Total 20%", "Total 10%", "Total 5%", "Total 0%"), "Total", "Imponible")#Creo una nueva variable para distinguir el IVA base del imponible
